...
class FlightController:
    ...
    def get_home_gps(self):
        ...
        if self._home_pos:
            return (
                self._home_pos.geo.latitude,
                self._home_pos.geo.longitude,
                self._home_pos.geo.altitude
            )
        return None
    ...
    def execute_waypoints(self, waypoints, home_gps=None):
        ...
        Args:
            waypoints: 航点列表，每个航点包含 lat, lng, alt
            home_gps: Home点GPS坐标 (lat, lon, alt)，用于坐标转换
        ...
        for i, wp in enumerate(waypoints):
            ...
            #这里区分使用gps坐标还是转成enu坐标进行飞行控制，默认使用gps坐标进行飞行控制
            if self.debug_setPosition_gps:
                rospy.loginfo("[Flight] (DEBUG) Simulating fly_to GPS (%.6f, %.6f, %.1f)", lat, lon, alt)
                rospy.sleep(2.0)
            else:
